...
実装の作業単位は [番号付きチケット一覧](tickets/README.md) に分割しました。設計の原本はこの計画書、実施範囲・依存関係・受け入れ条件は各チケットを参照します。[新しいセッションからの自動実装手順](development-workflow.md) に従って、1チケットごとにブランチ・レビュー・検証・Draft PR を作成します。
...
- 趣味開発中は Chrome を主対象とし、Edge / Firefox は本格公開の準備開始後に検証する。
...
| M5: 公開品質     | Chrome 実機確認、操作性、将来の横断・長時間検証手順                                 | 趣味開発中はオーナー確認と自動回帰が成功し、公開準備用ランブックがある               |
...
コンテナ内の headless 検証だけで、実際の音切れ・遅延・音質の合格とはしない。趣味開発中はオーナーの Chrome 手動確認を受け入れに使い、報告の範囲と未記録の詳細を明示する。未提供の値は推測しない。

本格公開の準備を始めた時点で、Windows の Chrome / Edge / Firefox を使い、CPU、ブラウザ版、音声デバイス、sampleRate、baseLatency / outputLatency（取得可能な場合）を記録する。

公開準備時の性能目標は、4気筒・48 kHz で10分間の連続動作中に知覚できる音切れがなく、操作から可聴応答まで50 ms以内。これは保証値ではなく将来の測定目標で、Bluetooth などの出力遅延を分けて記録する。開度による機械的な回転追従時間と、入力から処理へ届く遅延も分ける。
...
## 14. 追加改善: レイアウト・日本語・排気量・高回転域

2026-09-27の要望を既存コードに照らし、M6の追加チケット27〜31に分割する。既存M6チケット22〜24の完了条件は変更しない。

- **27: レスポンシブ/PCダッシュボード。** §7の縦配置を拡張し、PCでは操作と可視化を複数列へ、狭幅では単列へ配置する。1366×768と1440×900では詳細を閉じた主要操作・可視化を一画面に収め、320px幅から横overflowなしで利用可能にする。詳細展開や拡大時は縦スクロールを許容する。
- **28: 日英i18n。** 27の構造を前提に、表示層へja/en辞書を導入する。保存済み選択、ブラウザ言語、en fallbackの順で初期化し、表示・エラー・支援技術向け文言を切り替える。数値モデルとWorkletの識別子は言語非依存のままとする。
- **29: 排気量モデル。** §4の設定へ総排気量とトルク曲線の基準総排気量[cc]を追加し、駆動トルクへ両者の比を一度だけ反映する。等排気量の気筒を前提とし、慣性/摩擦/idle補助は独立設定に保つ。旧構成の両値欠落は明示したモデル値で倍率1へ正規化する。全プリセットは実測値を主張しない。
- **30: 排気量UI/音声。** 28と29の後に停止中の排気量適用と翻訳を統合する。1気筒排気量から有界な燃焼パルス形状を導出し、固定RPMの燃焼頻度は変えない。音量だけでなく波形/スペクトルの変化を自動検証する。
- **31: 高回転域。** 現行12,000 rpmと毎秒800燃焼イベントの独立した上限を検証し、16,000 rpm以上の単気筒・4気筒4ストローク構成を目標に対応範囲を拡張する。8気筒など高密度構成はイベント密度とWorklet処理予算で判定し、上限を無条件に緩和しない。

実装順序は27→28と29を独立レーンとし、28+29→30で合流する。31は24の完了後に独立して実装可能とする。各チケットの依存先はindexを原本とし、着手時にverified/mergedを確認する。排気量の上下限・互換入力の基準値・DSP係数は29/30の実装時に根拠と境界テストとともに確定し、この節へ追記する。実機・聴感は任意の補助証跡とし、自動回帰とRepository checksは必須とする。
