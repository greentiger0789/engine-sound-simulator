...
- 位相積分器の入力は sample ごとの角速度（rad/s）とし、dynamics の1 kHz更新との接続はチケット9で行う。更新間は直近の角速度を保持し、buffer終端のRPMを過去の全sampleへ遡及適用しない。
- 絶対frameとcycleの連続性を保持し、eventを時刻順に出す。同時刻だけcylinder idで安定順序を付ける。0 rpmではeventを出さず、開始角0°を含む半開区間と次sampleへの境界帰属をテストする。
- 固定回転だけでなく、同一sample位置で角速度を変更する入力列を44.1/48 kHz・異なるbuffer分割で検証する。異常な角速度やevent密度で無制限ループにならない上限と拒否動作を定義する。
