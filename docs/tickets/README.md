...
各作業ブランチは `feat/ticket-NNN-slug`、報告は `reports/NNN.md` を使います。報告は実装 PR 内で追加し、自動検証コマンドと結果、レビューした実装コミット、未解決事項を記録します。PR の最終 head SHA と CI URL は PR 本文に記載します。将来の merge commit を事前に書く必要はありません。人の Windows/音声確認は自動検証とは別欄です。現在の趣味開発段階では、オーナーの明示的な「問題なし」報告を、実際に報告された手動操作の受け入れ証跡として扱います。未提供の環境値や測定値は記録なしと明示し、推測しません。クロスブラウザ、長時間負荷、物理遅延は、オーナーが公開準備を始めるまで任意です。コンテナやheadless E2Eは実機証跡に読み替えず、別の自動回帰証跡として記録します。
